...
# =============================================================================
# PERÍCIAS D&D 5E (18 perícias oficiais) — FONTE CANÔNICA
# =============================================================================
...
# =============================================================================
@dataclass
class ResultadoSalvacaoMorte:
    """Resultado de um salvamento contra morte (Death Save) em D&D 5e.

    Mantém compatibilidade com acesso estilo dict (``res["estabilizou"]`` e
    ``res.get("hp_recuperado")``) para não quebrar código legado como o
    ``combat_logic.py``.
    """
    sucesso: bool = False
    falha: bool = False
    estabilizou: bool = False
    morreu: bool = False
    novos_sucessos: int = 0
    novas_falhas: int = 0
    hp_recuperado: Optional[int] = None

    def __getitem__(self, key: str):
        """Compatibilidade com acesso estilo dict (``res['estabilizou']``)."""
        return getattr(self, key)

    def get(self, key: str, default=None):
        """Compatibilidade com ``dict.get()``."""
        return getattr(self, key, default)

    def as_dict(self) -> Dict[str, Any]:
        """Retorna a representação em dict (para serialização/JSON)."""
        return asdict(self)


def calcular_salvacao_morte(rolagem: int, sucessos: int = 0, falhas: int = 0) -> ResultadoSalvacaoMorte:
    """
    Calcula resultado de salvacao de morte conforme D&D 5e.
    Retorna um ``ResultadoSalvacaoMorte`` com campos tipados do resultado.
    """
    resultado = ResultadoSalvacaoMorte(
...
def aplicar_modificadores_dano(dano_base: int, tipo_dano: str, 
                               vulnerabilidades: Optional[List[str]] = None,
                               resistencias: Optional[List[str]] = None,
                               imunidades: Optional[List[str]] = None) -> int:
    """
